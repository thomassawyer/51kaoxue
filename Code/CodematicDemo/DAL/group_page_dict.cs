using System;
using System.Data;
using System.Text;
using System.Data.SqlClient;
using Maticsoft.DBUtility;//Please add references
namespace Maticsoft.DAL
{
	/// <summary>
	/// 数据访问类:group_page_dict
	/// </summary>
	public partial class group_page_dict
	{
		public group_page_dict()
		{}
		#region  BasicMethod

		/// <summary>
		/// 是否存在该记录
		/// </summary>
		public bool Exists(string GROUPID,string PAGEID)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from group_page_dict");
			strSql.Append(" where GROUPID=@GROUPID and PAGEID=@PAGEID ");
			SqlParameter[] parameters = {
					new SqlParameter("@GROUPID", SqlDbType.VarChar,50),
					new SqlParameter("@PAGEID", SqlDbType.VarChar,15)			};
			parameters[0].Value = GROUPID;
			parameters[1].Value = PAGEID;

			return DbHelperSQL.Exists(strSql.ToString(),parameters);
		}


		/// <summary>
		/// 增加一条数据
		/// </summary>
		public bool Add(Maticsoft.Model.group_page_dict model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into group_page_dict(");
			strSql.Append("GROUPID,PAGEID)");
			strSql.Append(" values (");
			strSql.Append("@GROUPID,@PAGEID)");
			SqlParameter[] parameters = {
					new SqlParameter("@GROUPID", SqlDbType.VarChar,50),
					new SqlParameter("@PAGEID", SqlDbType.VarChar,15)};
			parameters[0].Value = model.GROUPID;
			parameters[1].Value = model.PAGEID;

			int rows=DbHelperSQL.ExecuteSql(strSql.ToString(),parameters);
			if (rows > 0)
			{
				return true;
			}
			else
			{
				return false;
			}
		}
		/// <summary>
		/// 更新一条数据
		/// </summary>
		public bool Update(Maticsoft.Model.group_page_dict model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update group_page_dict set ");
#warning 系统发现缺少更新的字段，请手工确认如此更新是否正确！ 
			strSql.Append("GROUPID=@GROUPID,");
			strSql.Append("PAGEID=@PAGEID");
			strSql.Append(" where GROUPID=@GROUPID and PAGEID=@PAGEID ");
			SqlParameter[] parameters = {
					new SqlParameter("@GROUPID", SqlDbType.VarChar,50),
					new SqlParameter("@PAGEID", SqlDbType.VarChar,15)};
			parameters[0].Value = model.GROUPID;
			parameters[1].Value = model.PAGEID;

			int rows=DbHelperSQL.ExecuteSql(strSql.ToString(),parameters);
			if (rows > 0)
			{
				return true;
			}
			else
			{
				return false;
			}
		}

		/// <summary>
		/// 删除一条数据
		/// </summary>
		public bool Delete(string GROUPID,string PAGEID)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from group_page_dict ");
			strSql.Append(" where GROUPID=@GROUPID and PAGEID=@PAGEID ");
			SqlParameter[] parameters = {
					new SqlParameter("@GROUPID", SqlDbType.VarChar,50),
					new SqlParameter("@PAGEID", SqlDbType.VarChar,15)			};
			parameters[0].Value = GROUPID;
			parameters[1].Value = PAGEID;

			int rows=DbHelperSQL.ExecuteSql(strSql.ToString(),parameters);
			if (rows > 0)
			{
				return true;
			}
			else
			{
				return false;
			}
		}


		/// <summary>
		/// 得到一个对象实体
		/// </summary>
		public Maticsoft.Model.group_page_dict GetModel(string GROUPID,string PAGEID)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 GROUPID,PAGEID from group_page_dict ");
			strSql.Append(" where GROUPID=@GROUPID and PAGEID=@PAGEID ");
			SqlParameter[] parameters = {
					new SqlParameter("@GROUPID", SqlDbType.VarChar,50),
					new SqlParameter("@PAGEID", SqlDbType.VarChar,15)			};
			parameters[0].Value = GROUPID;
			parameters[1].Value = PAGEID;

			Maticsoft.Model.group_page_dict model=new Maticsoft.Model.group_page_dict();
			DataSet ds=DbHelperSQL.Query(strSql.ToString(),parameters);
			if(ds.Tables[0].Rows.Count>0)
			{
				return DataRowToModel(ds.Tables[0].Rows[0]);
			}
			else
			{
				return null;
			}
		}


		/// <summary>
		/// 得到一个对象实体
		/// </summary>
		public Maticsoft.Model.group_page_dict DataRowToModel(DataRow row)
		{
			Maticsoft.Model.group_page_dict model=new Maticsoft.Model.group_page_dict();
			if (row != null)
			{
				if(row["GROUPID"]!=null)
				{
					model.GROUPID=row["GROUPID"].ToString();
				}
				if(row["PAGEID"]!=null)
				{
					model.PAGEID=row["PAGEID"].ToString();
				}
			}
			return model;
		}

		/// <summary>
		/// 获得数据列表
		/// </summary>
		public DataSet GetList(string strWhere)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select GROUPID,PAGEID ");
			strSql.Append(" FROM group_page_dict ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
			return DbHelperSQL.Query(strSql.ToString());
		}

		/// <summary>
		/// 获得前几行数据
		/// </summary>
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ");
			if(Top>0)
			{
				strSql.Append(" top "+Top.ToString());
			}
			strSql.Append(" GROUPID,PAGEID ");
			strSql.Append(" FROM group_page_dict ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
			strSql.Append(" order by " + filedOrder);
			return DbHelperSQL.Query(strSql.ToString());
		}

		/// <summary>
		/// 获取记录总数
		/// </summary>
		public int GetRecordCount(string strWhere)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) FROM group_page_dict ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
			object obj = DbHelperSQL.GetSingle(strSql.ToString());
			if (obj == null)
			{
				return 0;
			}
			else
			{
				return Convert.ToInt32(obj);
			}
		}
		/// <summary>
		/// 分页获取数据列表
		/// </summary>
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("SELECT * FROM ( ");
			strSql.Append(" SELECT ROW_NUMBER() OVER (");
			if (!string.IsNullOrEmpty(orderby.Trim()))
			{
				strSql.Append("order by T." + orderby );
			}
			else
			{
				strSql.Append("order by T.PAGEID desc");
			}
			strSql.Append(")AS Row, T.*  from group_page_dict T ");
			if (!string.IsNullOrEmpty(strWhere.Trim()))
			{
				strSql.Append(" WHERE " + strWhere);
			}
			strSql.Append(" ) TT");
			strSql.AppendFormat(" WHERE TT.Row between {0} and {1}", startIndex, endIndex);
			return DbHelperSQL.Query(strSql.ToString());
		}

		/*
		/// <summary>
		/// 分页获取数据列表
		/// </summary>
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			SqlParameter[] parameters = {
					new SqlParameter("@tblName", SqlDbType.VarChar, 255),
					new SqlParameter("@fldName", SqlDbType.VarChar, 255),
					new SqlParameter("@PageSize", SqlDbType.Int),
					new SqlParameter("@PageIndex", SqlDbType.Int),
					new SqlParameter("@IsReCount", SqlDbType.Bit),
					new SqlParameter("@OrderType", SqlDbType.Bit),
					new SqlParameter("@strWhere", SqlDbType.VarChar,1000),
					};
			parameters[0].Value = "group_page_dict";
			parameters[1].Value = "PAGEID";
			parameters[2].Value = PageSize;
			parameters[3].Value = PageIndex;
			parameters[4].Value = 0;
			parameters[5].Value = 0;
			parameters[6].Value = strWhere;	
			return DbHelperSQL.RunProcedure("UP_GetRecordByPage",parameters,"ds");
		}*/

		#endregion  BasicMethod
		#region  ExtensionMethod

		#endregion  ExtensionMethod
	}
}

